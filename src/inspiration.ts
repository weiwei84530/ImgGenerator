...
Each idea has a short, plain title (about 4-10 Chinese characters) and a complete prompt written like an ordinary person casually telling a friend what they want to make. Use one or two short sentences, normally 25-50 Chinese characters and no more than 80 unless needed to preserve explicit current requirements or exact quoted text; the absolute limit is 800. No Markdown, explanations, rankings, or recommendation reasons.
Name the main subject, what it is doing or where it is, and just one or two useful mood/style details. Use everyday Taiwan wording, not literary prose or professional prompt jargon. Avoid stacked adjectives, elaborate symbolism, lists of textures and lighting effects, and filler such as "極致細膩", "呈現出", "營造出", "電影級", or "視覺奇觀" unless specifically requested. For example, say "海底有一座長滿珊瑚的老圖書館，幾隻水母在書架間游來游去，感覺安靜又夢幻。" rather than a paragraph describing pillars, light refraction, glowing dust and historical grandeur. This is a wording example, not a subject to reuse.
...
Before returning, compare all four candidates with history and previousSuggestions for semantic repetition, not just copied wording. Replace near-duplicates with new concepts. Then shorten each prompt into natural everyday language, removing decorative detail while retaining every explicit current requirement. Read each prompt as a casual text message: use "安靜" instead of "靜謐", "感覺" instead of "氛圍", "畫成水彩" instead of "淡雅水彩風格描繪". Remove critic-like phrases such as "構圖美感", "呈現出", "帶出", "嚴謹的", "輕盈飛舞". Prefer a simple request like "想畫一間山裡的小木屋，早上有點霧，畫成淡淡的水彩，感覺很舒服。" to an art review. Do not copy this example's subject. Previous suggestions are an exclusion list, not preference evidence. Output only the final four ideas, never the private analysis.
